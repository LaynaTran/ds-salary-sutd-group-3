...
├── requirements.txt
├── data/
│   └── salary_cleaned.csv                  # Source dataset (110,526 rows; 99,425 after cleaning)
├── src/
│   ├── modeling/                           # Main part — core model development
│   │   ├── train.py                        # End-to-end training pipeline
│   │   └── predict.py                      # CLI for generating predictions from the saved model
│   └── data_generation/                    # Secondary part — supplementary
│       └── generate_synthetic_viz_data.py  # Generates a synthetic grid scored by the final model
└── outputs/                                # Generated artifacts
    ├── model.pkl                           # Final trained pipeline (preprocessing + tuned XGBoost)
    ├── metrics.json                        # CV + held-out test metrics for all candidates
    ├── feature_importance.png              # Top 20 feature importances for the final model
    ├── residuals.png                       # Predicted-vs-actual and residual distribution plots
    └── synthetic_salary_data_for_viz.csv   # Output of the secondary component (Section 5)
```

| File | Purpose |
|---|---|
| `src/modeling/train.py` | Loads and cleans the data, benchmarks four model families via cross-validation, tunes the winner with `RandomizedSearchCV`, evaluates on a held-out test set, and writes all artifacts to `outputs/`. |
| `src/modeling/predict.py` | Loads `outputs/model.pkl` and scores new inputs, either as CLI flags for a single row or a CSV for batch predictions. |
| `src/data_generation/generate_synthetic_viz_data.py` | Supplementary script (Section 5): scores a full factorial grid of profiles with the final model for visualization purposes. |
| `outputs/model.pkl` | The only artifact needed to serve predictions — bundles preprocessing (encoding) and the tuned XGBoost model into one deployable object. |
| `outputs/metrics.json` | Machine-readable record of every candidate's cross-validation and final test metrics, backing the table in [Section 1](#1-project-summary--final-results). |
...
### Generate the synthetic visualization dataset (secondary part)
